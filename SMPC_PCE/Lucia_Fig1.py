...
for k in range(N):
    ...
    mean_state = G_big @ X[:, k]
    V = controller.var_x(X[:, k])
    variance_sqrt = cp.sqrt(V)  # Vの平方根を計算
    chance_constraint = mean_state + cp.multiply(kappa * G_big, variance_sqrt) 
    constraints += [chance_constraint <= g_big]
    constraints += [H_big[1] @ U[:, k] <= h_big]
    # constraints += [H_big[2] @ U[:, k] <= h_big]
